...
class Event_Manager:
          ...
          def update_window_events(self):
                    for event in pygame.event.get():
                              if event.type == pygame.QUIT or self.game.keys[pygame.K_ESCAPE]: self.game.running = False
          ...
          def update_fps_toggle(self):
                    if self.game.keys[TOGGLE_FPS_KEY] and self.game.Last_Event + self.FPS_Cooldown < time.time():
