...
class EventManager:
          ...
          def fullscreen_toggle(self):
                    if self.game.keys[Keys['fullscreen']] and self.Last_Fullscreen + self.Fullscreen_Cooldown < pygame.time.get_ticks() / 1000:
                              pygame.display.toggle_fullscreen()
                              self.Last_Fullscreen = pygame.time.get_ticks() / 1000

          def update_fps_toggle(self):
                    if self.game.keys[Keys['fps']] and self.Last_FPS_Toggle + self.FPS_Cooldown < pygame.time.get_ticks() / 1000 and not self.game.in_menu:
          ...
          def update_changing_settings(self):
                    if self.game.keys[Keys['ungrab']] and self.Last_Changing_settings + self.Changing_settings_Cooldown < pygame.time.get_ticks() / 1000 and not self.game.in_menu:
