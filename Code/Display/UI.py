...
class UI:
          ...
          def draw_bars(self):
                    ...
                    if self.game.player.stamina > 0: stamina = self.game.player.stamina
                    else: stamina = 1
                    Stamina_bar_surface = pygame.Surface((self.stamina_bar_rect.width * stamina / PLAYER_STAMINA, self.stamina_bar_rect.height))
          ...
          def draw_brightness(self):
                    ...
                    if self.brightness > 50: self.game.display.fill([int(MAX_BRIGHTNESS * (self.brightness - 50)) for _ in range(3)], special_flags=pygame.BLEND_RGB_ADD)
                    elif self.brightness < 50: self.game.display.fill([int(MAX_BRIGHTNESS * (50 - self.brightness)) for _ in range(3)], special_flags=pygame.BLEND_RGB_SUB)
