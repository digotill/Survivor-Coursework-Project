...
class UI:
          ...
          def draw_bars(self):
                    ...
                    (HEALTH_BAR_POS[0] - 0.5 * self.health_bar_rect.width) * REN_RES[0] / 640,
                    (HEALTH_BAR_POS[1] - 0.5 * self.health_bar_rect.height) * REN_RES[0] / 640 - 1))
                    self.game.ui_surface.blit(Outside_Health_bar, (
                    (HEALTH_BAR_POS[0] - 0.5 * Outside_Health_bar.get_rect().width) * REN_RES[0] / 640,
                    ...
                    self.game.ui_surface.blit(pygame.transform.flip(Outside_Health_bar, True, False),
                                              (REN_RES[0] - (STAMINA_BAR_POS[0] + 0.5 *
                                                             Outside_Health_bar.get_rect().width) * REN_RES[
                                                         0] / 640 - 2,
          ...
          def draw_time(self):
                    if self.fps_enabled:
                              ...
                              REN_RES[0] - TIME_POS[0] * REN_RES[0] / 640, TIME_POS[1] * REN_RES[0] / 640))
                              self.game.ui_surface.blit(text, text_rect)
          ...
          def draw_brightness(self):
                    ...
                    if self.brightness > 50:
                              self.game.display.fill([int(MAX_BRIGHTNESS * (self.brightness - 50)) for _ in range(3)],
                                                     special_flags=pygame.BLEND_RGB_ADD)
                    elif self.brightness < 50:
                              self.game.display.fill([int(MAX_DARKNESS * (50 - self.brightness)) for _ in range(3)],
