...
class EventManager:
          ...
          def update_size(self, always_toggle=False):
                    if self.game.keys[keys['fullscreen']] and self.Last_Fullscreen + self.Fullscreen_Cooldown < pygame.time.get_ticks() / 1000 or always_toggle:
          ...
          def update_fps_toggle(self):
                    if self.game.keys[keys['fps']] and self.Last_FPS_Toggle + self.FPS_Cooldown < pygame.time.get_ticks() / 1000:
          ...
          def update_grab(self):
                    ...
                    elif self.game.keys[keys['ungrab']]: pygame.event.set_grab(False)

          def update_changing_settings(self):
                    if self.game.keys[keys['ungrab']] and self.Last_Changing_settings + self.Changing_settings_Cooldown < pygame.time.get_ticks() / 1000 and not self.game.mainmenu.in_menu:
